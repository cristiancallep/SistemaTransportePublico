...
@Component({
  ...
  imports: [
    ...
    .filter-input, .filter-select {
        height: 30px; padding: 6px 8px; border-radius:6px; border:1px solid #d0d0d0; background: #fff; font-size:0.95rem; outline:none; box-sizing:border-box; transition: box-shadow 0.12s ease, border-color 0.12s ease;
    }
...
export class UsuariosListComponent implements OnInit {
    ...
    buscarPorDocumento(documento: string): void {
        ...
        if (!doc) { this.snackBar.open('Ingrese un documento', 'Cerrar', { duration: 2000 }); return; }
        this.usuarioService.getUsuarioByDocumento(doc).subscribe({
    ...
    buscarPorEmail(email: string): void {
        ...
        if (!em) { this.snackBar.open('Ingrese un email', 'Cerrar', { duration: 2000 }); return; }
        this.usuarioService.getUsuarioByEmail(em).subscribe({
            next: (u: Usuario) => { this.dataSource.data = [u]; this.total = 1; },
            error: (err: any) => {
